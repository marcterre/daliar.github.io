...
const TextBlock: React.FC<TextBlockProps> = ({ block }) => {
  ...
  const getZIndex = () => {
    if (isDragging) return 9999;
    if (isEditMode) return Math.max(block.zIndex, 100);
    return block.zIndex;
  };
  ...
  const maxWidth = `calc(100vw - 100px)`;
      ...
      <div className="relative inline-flex items-center">
        {block.isEditing ? (
          <input
            ...
            className="bg-white border border-blue-400 rounded outline-none"
        ...
        {showControls && (
          <Button
            type="black"
            handleClick={(e: React.MouseEvent) => handleDelete(e)}
            handleMouseEnter={() => setIsControlsHovered(true)}
            handleMouseLeave={() => setIsControlsHovered(false)}
            handleMouseDown={(e: React.MouseEvent) => {
              e.stopPropagation();
              e.preventDefault();
            }}
            icon={<Trash2 size={14} />}
            className="m-0 ml-2 px-2"
          />
        )}
      </div>
